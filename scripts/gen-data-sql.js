...
// Hàm để convert giá trị thành JSON string
function toJson(val) {
...
const fixedProducts = products.map((product) => {
  let originalValue = product.id;
  let newValue = originalValue;
  let counter = 1;

  // Kiểm tra trùng và tạo ID mới nếu bị trùng
  while (seen.has(newValue)) {
    newValue = `${originalValue}-${counter++}`;
  }

  if (newValue !== originalValue) {
    console.warn(`⚠️ Trùng id: "${originalValue}" → sửa thành "${newValue}"`);
  }

  seen.set(newValue, true);
...
const sqlHeader = `INSERT INTO products (
...
const sqlValues = products.map(p => {
  const product_id = escapeString(p.id);
  const id = escapeString(p.id);
  const url = escapeString(p.imageUrl);
...
fs.writeFileSync('import-products.sql', sqlHeader + sqlValues + sqlFooter);
console.log("Length of products:", products.length);
console.log("✅ Đã tạo file import-products.sql thành công.");
