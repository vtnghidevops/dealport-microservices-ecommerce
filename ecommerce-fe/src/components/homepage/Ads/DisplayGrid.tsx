...
const DisplayCard: React.FC<DisplayItem> = ({ image, title, price, discount_img, buttonType }) => {
  ...
  if (title && price) {
    ...
    return (
      <div className="rounded-lg h-[146px] w-[188px] overflow-hidden shadow-md transition-all hover:shadow-xl">
        <CardSecond image={image} buttonType={buttonType} />
      </div>
    );
  } else {
...
const CardFirst: React.FC<CardFirst> = ({ image }) => {
  return (
    <a href="#" className="relative h-[100%] block">
      <img src={image} className="absolute rounded-xl "></img>
      <a
        href="#"
...
const CardSecond: React.FC<CardSecond> = ({ image, buttonType }) => {
  return (
...
const CardMain: React.FC<CardMain> = ({ image, title, price, discount_img, buttonType }) => {
  return (
