    ...
    try {
      ...
      if (response.data && response.data.data) {
        const transactions: Transaction[] = response.data.data.map((order: any) => ({
          id: order.id,
          customerId: `#${order.customer_id.substring(0, 5)}`,
        ...
        return {
          transactions,
          total: response.data.meta?.total_items || transactions.length
        };
      }
...
function getFallbackTransactionData(page: number, limit: number): TransactionListResponse {
