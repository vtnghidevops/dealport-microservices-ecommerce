...
const ProductListPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        if (urlSearchQuery && !categorySlug) {
          // Search mode - search all products
          const { products: searchResults } =
            await ProductService.getAllProducts(1, 100, {
          ...
          const productsData = await ProductService.getProductsByCategorySlug(
            categorySlug
          );
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    ProductService.getProductsByCategorySlug(categorySlug).then((products) => {
      ...
      const filtered = products.products.filter((product) => {
        ...
        return searchIn.some((text) => text.includes(normalizeText(query)));
      });
    ...
    try {
      ...
      if (selectedTag) {
        ...
        filteredProducts = filteredProducts.filter((product: Product) =>
          product.tags?.some(
            (tag) => normalizeText(tag) === normalizedSelectedTag
          )
        );
      }
      ...
      switch (sortOption) {
        ...
        case "rating":
          filteredProducts.sort(
            (a, b) => (b.reviewsAvg?.rating || 0) - (a.reviewsAvg?.rating || 0)
          );
          break;
        default:
